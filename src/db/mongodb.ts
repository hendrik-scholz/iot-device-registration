...
function connectToMongoDbForConnectionString(connectionString: string): Promise<Mongoose> {
...
function saveDevice(device: Device): Promise<mongoose.Document> {
    const deviceModel = new DeviceModel({
        ...
        geoposition: {
            ...
            coordinates: [device.geoPosition.coordinates.longitude, device.geoPosition.coordinates.latitude]
        },
        identification: {
...
function getDevices(): Promise<Device[]> {
    ...
    pipeline.push({ $project: {
        ...
        'geoPosition.coordinates.longitude': { $arrayElemAt: ['$geoposition.coordinates', 0] },
        'geoPosition.coordinates.latitude': { $arrayElemAt: ['$geoposition.coordinates', 1] },
...
export { connectToMongoDbForConnectionString, disconnectFromMongoDb, saveDevice, getDevices, getDeviceForUuid, getDevicesInGeofence }
