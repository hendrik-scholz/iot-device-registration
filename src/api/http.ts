...
import { getDevices_, getDeviceForUuid_, getDevicesInGeofence_ } from '../services/deviceService';
...
function startService(): void {
    ...
    app.get('/devices', (req: Request, res: Response) => {
        getDevices_()
            .then(devices => { res.append('Access-Control-Allow-Origin', '*'); res.json(devices); })
            .catch(error => res.sendStatus(500));
    });
    app.get('/devices/geofence', (req: Request, res: Response) => {
        if (req && req.query && req.query.lat && req.query.lng && req.query.radius) {
    ...
    app.listen(port, () => logger.info(`iot-device-registry app listening on port ${port}!`));
}
